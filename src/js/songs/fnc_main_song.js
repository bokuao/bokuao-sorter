...
function fnc_GetSavedDataVersion() {
  const version = $.jStorage.get("BokuAoSongSorter_saveVersion");
  if (version) {
    return version;
  }

  const hasOldData = $.jStorage.get("BokuAoSongSorter_int_Total") !== null;
  if (hasOldData) {
...
function startup() {
  getID("middle-actions-end").style.display = "none";
  getID("fldLeft").style.display = "none";
  getID("texLeft").style.display = "none";
  getID("fldRight").style.display = "none";
  getID("texRight").style.display = "none";
...
//   var elms = document.querySelectorAll(".opt_toggleAll");
...
// * Only happens once
function init_song() {
  getID("fldLeft").style.display = "flex";
  getID("texLeft").style.display = "flex";
  getID("fldRight").style.display = "flex";
  getID("texRight").style.display = "flex";

  int_Total = 0;
  int_RecordID = 0;
  var arranges = getID("optArrange").checked;
  var sortTypes =
    getID("optSortType").options[getID("optSortType").selectedIndex].value;
  ...
  for (let i = 0; i < ary_SongData.length; i++) {
    ...
    if (anyIntersection(ary_SongData[i][TRACK_TITLES], selectionSet)) {
      ...
      const correctArrangementType =
        arranges || ary_SongData[i][TRACK_IS_ARRANGEMENT] === ORIGINAL_TRACK;
...
function fnc_Save() {
  ...
  $.jStorage.set(
    "BokuAoSongSorter_saveVersion",
    SAVE_VERSION.ReduceSongData,
    null
  );

  $.jStorage.set("BokuAoSongSorter_ary_EqualData", ary_EqualData, null);
  $.jStorage.set("BokuAoSongSorter_ary_ParentData", ary_ParentData, null);
  $.jStorage.set("BokuAoSongSorter_ary_RecordData", ary_RecordData, null);
  $.jStorage.set("BokuAoSongSorter_ary_SortData", ary_SortData, null);
  $.jStorage.set("BokuAoSongSorter_ary_TempData", ary_TempData, null);

  $.jStorage.set(
    "BokuAoSongSorter_back_ary_EqualData",
    back_ary_EqualData,
    null
  );
  $.jStorage.set(
    "BokuAoSongSorter_back_ary_ParentData",
    back_ary_ParentData,
    null
  );
  $.jStorage.set(
    "BokuAoSongSorter_back_ary_RecordData",
    back_ary_RecordData,
    null
  );
  $.jStorage.set("BokuAoSongSorter_back_ary_SortData", back_ary_SortData, null);
  $.jStorage.set(
    "BokuAoSongSorter_back_int_Completed",
    back_int_Completed,
    null
  );

  $.jStorage.set("BokuAoSongSorter_back_int_LeftID", back_int_LeftID, null);
  $.jStorage.set("BokuAoSongSorter_back_int_LeftList", back_int_LeftList, null);
  $.jStorage.set("BokuAoSongSorter_back_int_RecordID", back_int_RecordID, null);
  $.jStorage.set("BokuAoSongSorter_back_int_RightID", back_int_RightID, null);
  $.jStorage.set(
    "BokuAoSongSorter_back_int_RightList",
    back_int_RightList,
    null
  );

  $.jStorage.set("BokuAoSongSorter_back_int_Total", back_int_Total, null);
  $.jStorage.set("BokuAoSongSorter_int_Completed", int_Completed, null);
  $.jStorage.set("BokuAoSongSorter_int_Count", int_Count, null);
  $.jStorage.set("BokuAoSongSorter_int_LeftID", int_LeftID, null);
  $.jStorage.set("BokuAoSongSorter_int_LeftList", int_LeftList, null);

  $.jStorage.set("BokuAoSongSorter_int_RecordID", int_RecordID, null);
  $.jStorage.set("BokuAoSongSorter_int_RightID", int_RightID, null);
  $.jStorage.set("BokuAoSongSorter_int_RightList", int_RightList, null);
  $.jStorage.set("BokuAoSongSorter_int_Status", int_Status, null);
  $.jStorage.set("BokuAoSongSorter_int_Total", int_Total, null);
...
function fnc_Load() {
  ...
  if (saveVersion !== null) {
    ...
    ary_EqualData = $.jStorage.get("BokuAoSongSorter_ary_EqualData");
    ary_ParentData = $.jStorage.get("BokuAoSongSorter_ary_ParentData");
    ary_RecordData = $.jStorage.get("BokuAoSongSorter_ary_RecordData");
    ary_SortData = $.jStorage.get("BokuAoSongSorter_ary_SortData");
    ary_TempData = $.jStorage.get("BokuAoSongSorter_ary_TempData");

    back_ary_EqualData = $.jStorage.get("BokuAoSongSorter_back_ary_EqualData");
    back_ary_ParentData = $.jStorage.get(
      "BokuAoSongSorter_back_ary_ParentData"
    );
    back_ary_RecordData = $.jStorage.get(
      "BokuAoSongSorter_back_ary_RecordData"
    );
    back_ary_SortData = $.jStorage.get("BokuAoSongSorter_back_ary_SortData");
    back_int_Completed = $.jStorage.get("BokuAoSongSorter_back_int_Completed");

    back_int_LeftID = $.jStorage.get("BokuAoSongSorter_back_int_LeftID");
    back_int_LeftList = $.jStorage.get("BokuAoSongSorter_back_int_LeftList");
    back_int_RecordID = $.jStorage.get("BokuAoSongSorter_back_int_RecordID");
    back_int_RightID = $.jStorage.get("BokuAoSongSorter_back_int_RightID");
    back_int_RightList = $.jStorage.get("BokuAoSongSorter_back_int_RightList");

    back_int_Total = $.jStorage.get("BokuAoSongSorter_back_int_Total");
    int_Completed = $.jStorage.get("BokuAoSongSorter_int_Completed");
    int_Count = $.jStorage.get("BokuAoSongSorter_int_Count");
    int_LeftID = $.jStorage.get("BokuAoSongSorter_int_LeftID");
    int_LeftList = $.jStorage.get("BokuAoSongSorter_int_LeftList");

    int_RecordID = $.jStorage.get("BokuAoSongSorter_int_RecordID");
    int_RightID = $.jStorage.get("BokuAoSongSorter_int_RightID");
    int_RightList = $.jStorage.get("BokuAoSongSorter_int_RightList");
    int_Status = $.jStorage.get("BokuAoSongSorter_int_Status");
    int_Total = $.jStorage.get("BokuAoSongSorter_int_Total");
...
function fnc_ShowResults() {
  ...
  for (let i = 0; i < ary_TempData.length; i++) {
    ...
    var textForEntry = "";
    if (!displayType) {
      textForEntry = obj_TempData[TRACK_NAME];
    } else {
      textForEntry = obj_TempData[TRACK_DESCRIPTION];
    }
  ...
  for (let i = 0; i < 2; i++) {
    ...
    if (getID("optImage").checked) {
      ...
      if (obj_TempData[TRACK_YOUTUBE_ID] != "na") {
        var isSpotify = obj_TempData[TRACK_YOUTUBE_ID].includes("SPOTIFY");
        var usedLink = isSpotify
          ? obj_TempData[TRACK_YOUTUBE_ID].split("|")
          : obj_TempData[TRACK_YOUTUBE_ID];
        var obj_Item = createElement("iframe");
        obj_Item.width = isSpotify ? "535" : "180";
        obj_Item.height = isSpotify ? "155" : "180";
        obj_Item.frameBorder = "0";
        obj_Item.src = isSpotify
          ? str_SpotEmbed + usedLink[1]
          : str_YouPath + usedLink;
...
function fnc_ShowData() {
  ...
  setClass(
    getID("fldMiddleB"),
    int_Completed === back_int_Completed ? "inactive" : null
  );

  var gaugePercentage = Math.floor((int_Completed * 100) / int_Total);
  getID("lblCount").innerHTML =
    int_Count + " (Possibly " + (int_Total - int_Completed) + " remaining)";
  getID("lblProgress").innerHTML = gaugePercentage;
  refreshGauge(sGaugeID, (int_Completed * 100) / int_Total);

  if (gaugePercentage == 100) {
    getID("middle-actions-run").style.display = "none";
    getID("middle-actions-end").style.display = "flex";
  }
...
export { startup, fnc_Sort, fnc_Undo, fnc_Save, fnc_Load, fnc_ShowData };
